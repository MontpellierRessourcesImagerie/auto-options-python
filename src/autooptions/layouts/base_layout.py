...
class BaseLayout(QVBoxLayout):
    ...
    def addToLayout(
        ...
        raise NotImplementedError(
            "addToLayout method must be implemented by subclasses"
        )

    def applySizingStrategy(
        self, optionalCheckbox=None, nameLabel=None, valueField=None, tailWidget=None
    ):
        if self.sizingStrategy == "fixed":
            self.applyFixedSizingStrategy(
                optionalCheckbox, nameLabel, valueField, tailWidget
            )
        elif self.sizingStrategy == "auto":
            self.applyAutoSizingStrategy(
                optionalCheckbox, nameLabel, valueField, tailWidget
            )
        else:
            raise ValueError(f"Unknown sizing strategy: {self.sizingStrategy}")

    def applyFixedSizingStrategy(
        self, optionalCheckbox=None, nameLabel=None, valueField=None, tailWidget=None
    ):
    ...
    def applyAutoSizingStrategy(
        self, optionalCheckbox=None, nameLabel=None, valueField=None, tailWidget=None
    ):
        if optionalCheckbox is not None:
            optionalCheckbox.setSizePolicy(
